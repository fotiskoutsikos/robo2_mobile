...
class mymobibot_follower():
    ...
    def __init__(self,rate):
        ...
        self.mobibot_cotroller = controller.mobibot_controller()
    ...
    def get_sonar_measurements(self):
        sonar_F = self.sonar_F.range
        sonar_FL = self.sonar_FL.range
        sonar_FR = self.sonar_FR.range
        sonar_L = self.sonar_L.range
        sonar_R = self.sonar_R.range
    ...
    def publish(self):
        ...
        while not rospy.is_shutdown():
            ...
            sonar_measurements = self.get_sonar_measurements()
            self.mobibot_cotroller.update_sonar_measurements(sonar_measurements)
            self.mobibot_cotroller.update_time(rospy.get_rostime().to_sec())
            velocities = self.mobibot_cotroller.control_law()
            self.velocity.linear.x = velocities[0]
            self.velocity.angular.z = velocities[1]
    ...
    def turn_off(self):
        ...
        self.mobibot_cotroller.log_data()
